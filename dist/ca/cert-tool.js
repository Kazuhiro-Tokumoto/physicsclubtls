...
function verifyCAChain(cas, parentPubkey) {
    for (const ca of cas) {
...
export function verifyCertificate(cert, targetDomain) {
...
function findDomain(cert, targetDomain) {
    const searchCAs = (cas) => {
        for (const ca of cas) {
            if (ca.domain?.some((d) => d.domainname === targetDomain))
                return true;
            if (ca.CA) {
                if (searchCAs(ca.CA))
                    return true;
...
function listCAs(cas, prefix = "", results = [], currentPath = []) {
    cas.forEach((ca, i) => {
        ...
        if (ca.CA)
            listCAs(ca.CA, prefix + "  ", results, path);
    });
    return results;
}
function getCAByPath(root, path) {
    let ca = root.CA[path[0]];
    for (let i = 1; i < path.length; i++) {
        ca = ca.CA[path[i]];
    }
    return ca;
}
function clearEndFlags(cas) {
    for (const ca of cas) {
        delete ca.end;
        if (ca.CA)
            clearEndFlags(ca.CA);
...
function buildNameChain(root, path) {
    const names = [];
    let ca = root.CA[path[0]];
...
async function addToExisting() {
    ...
    if (addType === "1") {
        ...
        const domain = { domainname, pubkey, signday };
