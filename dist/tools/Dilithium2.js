class Dilithium2 {
    ...
    ETA = 2;
    GAMMA1 = 131072n; // 2^17
    GAMMA2 = 95232n; // (Q-1)/88
    OMEGA = 80;
    BETA = 78n; // TAU * ETA = 39 * 2
    TAU = 39;
    LAMBDA = 128;
    N_INV = 8347681n; // 256^{-1} mod Q
    ZETA = 1753n;
    ZETA_TABLE = [
    ...
    // Montgomery: R = 2^32
    MONT_R2 = 2365951n; // R^2 mod Q
    MONT_QINV = 4236238847n; // -Q^{-1} mod 2^32
    montReduce(a) {
    ...
    nttVecInner(a, b) {
        const r = new Array(this.N).fill(0n);
    ...
    polyPower2Round(p) {
        const high = new Array(this.N);
        const low = new Array(this.N);
        for (let i = 0; i < this.N; i++) {
    ...
    packEta(p) {
        ...
        for (let j = 0; j < this.N; j += 8) {
            ...
            for (let k = 0; k < 8; k++) {
                ...
                if (v < 0)
                    v += 5; // should not happen if coeffs in [-2,2]
                vals.push(v);
            }
    ...
    // ===== Key Serialization =====
    // pk = ρ (32) || t1_packed (1280) = 1312 bytes
    encodePk(pk) {
    ...
    // sk = ρ(32) || K(32) || tr(64) || s1(L*96=384) || s2(K*96=384) || t0(K*416=1664) = 2560 bytes
    encodeSk(sk) {
        const parts = [sk.rho, sk.K, sk.tr];
        for (let i = 0; i < this.L; i++)
            parts.push(this.packEta(sk.s1[i]));
        for (let i = 0; i < this.K; i++)
            parts.push(this.packEta(sk.s2[i]));
    ...
    decodeSk(buf) {
        ...
        const s1 = [];
        for (let i = 0; i < this.L; i++)
            s1.push(this.unpackEta(buf.slice(off, off += 96)));
        const s2 = [];
        for (let i = 0; i < this.K; i++)
            s2.push(this.unpackEta(buf.slice(off, off += 96)));
    ...
    keyGen(seed) {
        ...
        const expanded = this.shake256(this.concat(seed, new Uint8Array([this.K, this.L])), 128);
    ...
    useHint(hint, r) {
        ...
        const m = (this.Q - 1n) / (2n * this.GAMMA2); // 44
        if (hint === 0)
            return r1;
        if (r0 > 0n) {
            return (r1 + 1n) % m;
    ...
    sampleInBall(seed) {
        const c = new Array(this.N).fill(0n);
        const state = new Array(25).fill(0n);
    ...
    expandMask(rhoPrime, kappa) {
        const y = [];
        for (let i = 0; i < this.L; i++) {
            const input = this.concat(rhoPrime, new Uint8Array([(kappa + i) & 0xFF, ((kappa + i) >> 8) & 0xFF]));
            const buf = this.shake256(input, this.Z_POLY_BYTES); // 576 bytes
            const p = new Array(this.N);
            const mask = 0x3FFFF; // 18bit
            // 4係数 = 72bit = 9 bytes
            for (let j = 0; j < this.N; j += 4) {
                const base = (j >> 2) * 9;
                const v0 = (buf[base] | (buf[base + 1] << 8) | (buf[base + 2] << 16)) & mask;
                const v1 = ((buf[base + 2] >> 2) | (buf[base + 3] << 6) | (buf[base + 4] << 14)) & mask;
                const v2 = ((buf[base + 4] >> 4) | (buf[base + 5] << 4) | (buf[base + 6] << 12)) & mask;
                const v3 = ((buf[base + 6] >> 6) | (buf[base + 7] << 2) | (buf[base + 8] << 10)) & mask;
                p[j] = this.mod(this.GAMMA1 - BigInt(v0), this.Q);
                p[j + 1] = this.mod(this.GAMMA1 - BigInt(v1), this.Q);
                p[j + 2] = this.mod(this.GAMMA1 - BigInt(v2), this.Q);
                p[j + 3] = this.mod(this.GAMMA1 - BigInt(v3), this.Q);
            }
            y.push(p);
        }
        return y;
    ...
    // ===== Signature Packing =====
    // z: γ1=2^17 → 各係数18bit unsigned化, 4係数=72bit=9bytes
    // L=4 → 256*18/8 * 4 = 576 * 4 = 2304 bytes... いや
    // 256 * 18bit = 4608bit = 576 bytes per poly, L=4 → 2304 bytes
    Z_POLY_BYTES = 576; // N * 18 / 8
    packZ(z) {
        const out = new Uint8Array(this.L * this.Z_POLY_BYTES);
        let off = 0;
        const halfQ = this.Q >> 1n;
        const bits = 18; // log2(2 * GAMMA1) = log2(2^18) = 18
        const mask = (1 << bits) - 1;
        for (let i = 0; i < this.L; i++) {
            for (let j = 0; j < this.N; j += 4) {
                const vals = [];
                for (let k = 0; k < 4; k++) {
                    ...
                    if (zs > halfQ)
                        zs -= this.Q;
                    vals.push(Number(this.GAMMA1 - zs) & mask);
                }
                // 4 * 18bit = 72bit = 9 bytes
                out[off++] = vals[0] & 0xFF;
                out[off++] = (vals[0] >> 8) & 0xFF;
                out[off++] = ((vals[0] >> 16) | (vals[1] << 2)) & 0xFF;
                out[off++] = (vals[1] >> 6) & 0xFF;
                out[off++] = ((vals[1] >> 14) | (vals[2] << 4)) & 0xFF;
                out[off++] = (vals[2] >> 4) & 0xFF;
                out[off++] = ((vals[2] >> 12) | (vals[3] << 6)) & 0xFF;
                out[off++] = (vals[3] >> 2) & 0xFF;
                out[off++] = (vals[3] >> 10) & 0xFF;
            }
        }
        return out;
    }
    ...
    // w1: 各係数6bit (m=44→上位は0..43), 192 bytes per poly, K=4 → 768
    packW1(w1) {
        const out = new Uint8Array(this.K * 192); // 256 * 6 / 8 = 192
        let off = 0;
        for (let i = 0; i < this.K; i++) {
            for (let j = 0; j < this.N; j += 4) {
                const a = Number(w1[i][j]), b = Number(w1[i][j + 1]);
                const c = Number(w1[i][j + 2]), d = Number(w1[i][j + 3]);
                // 4 * 6bit = 24bit = 3 bytes
                out[off++] = (a | (b << 6)) & 0xFF;
                out[off++] = ((b >> 2) | (c << 4)) & 0xFF;
                out[off++] = ((c >> 4) | (d << 2)) & 0xFF;
            }
        }
        return out;
    }
    ...
    sign(skBytes, msg) {
        ...
        while (true) {
            ...
            // 5. z = y + c·s1
            const cs1 = this.vecInvNtt(s1Ntt.map(p => this.polyPointwise(cNtt.slice(), p)));
            const z = this.vecAdd(y, cs1);
            // 6. r0 = LowBits(w - c·s2)
            const cs2 = this.vecInvNtt(s2Ntt.map(p => this.polyPointwise(cNtt.slice(), p)));
            const wMinusCs2 = this.vecSub(w, cs2);
            const r0 = this.vecLowBits(wMinusCs2);
            // 7. Check ‖z‖∞ < γ1-β and ‖r0‖∞ < γ2-β
            if (!this.vecChkNorm(z, this.GAMMA1 - this.BETA))
                continue;
            if (!this.vecChkNorm(r0.map(p => p.map(c => this.mod(c, this.Q))), this.GAMMA2 - this.BETA))
                continue;
            // 8. Hint
            const ct0 = this.vecInvNtt(t0Ntt.map(p => this.polyPointwise(cNtt.slice(), p)));
    ...
    // ===== Verify =====
    verify(pkBytes, msg, sig) {
        const pk = this.decodePk(pkBytes);
        const A = this.expandA(pk.rho);
        // σ を分解: c̃(32) || z(L*576) || h(OMEGA+K)
        const zBytes = this.L * this.Z_POLY_BYTES;
        const cTilde = sig.slice(0, 32);
        const z = this.unpackZ(sig.slice(32, 32 + zBytes));
        const hints = this.unpackHint(sig.slice(32 + zBytes));
        if (!hints)
            return false;
        // ‖z‖∞ < γ1-β
        if (!this.vecChkNorm(z, this.GAMMA1 - this.BETA))
        ...
        // ct1·2^d
        const t1Ntt = this.vecNtt(pk.t1.map(p => p.map(c => this.mod(c * (1n << BigInt(this.D)), this.Q))));
        const ct1 = this.vecInvNtt(t1Ntt.map(p => this.polyPointwise(cNtt.slice(), p)));
    ...
    unpackZ(buf) {
        ...
        for (let i = 0; i < this.L; i++) {
            const p = new Array(this.N);
            for (let j = 0; j < this.N; j += 4) {
                const b = buf.slice(off, off + 9);
                off += 9;
                const v0 = (b[0] | (b[1] << 8) | (b[2] << 16)) & mask;
                const v1 = ((b[2] >> 2) | (b[3] << 6) | (b[4] << 14)) & mask;
                const v2 = ((b[4] >> 4) | (b[5] << 4) | (b[6] << 12)) & mask;
                const v3 = ((b[6] >> 6) | (b[7] << 2) | (b[8] << 10)) & mask;
                p[j] = this.mod(this.GAMMA1 - BigInt(v0), this.Q);
                p[j + 1] = this.mod(this.GAMMA1 - BigInt(v1), this.Q);
                p[j + 2] = this.mod(this.GAMMA1 - BigInt(v2), this.Q);
                p[j + 3] = this.mod(this.GAMMA1 - BigInt(v3), this.Q);
    ...
    pkFromSk(sk) {
        const { rho, s1, s2 } = this.decodeSk(sk);
        const A = this.expandA(rho);
        const s1Ntt = this.vecNtt(s1);
        const t = this.vecAdd(this.vecInvNtt(this.matVecNtt(A, s1Ntt)), s2);
        const { t1 } = this.vecPower2Round(t);
        return this.encodePk({ rho, t1 });
    }
}
...
function showToast(msg) {
    ...
    padding:10px 16px; background:#1a1917; color:#f7f6f3;
...
<div class="tabs" id="tab-bar">
  ...
  <button class="tab" data-tab="sktopk">sk→pk</button>
</div>
<div class="panels">
  ...
  <div id="panel-keygen" class="panel active">
    <div class="field">
      <label>seed (hex 64文字, 任意)</label>
      <input type="text" id="keygen-seed" placeholder="空欄でランダム生成">
    </div>
    ...
    <div class="field">
      ...
      <div class="output-wrap"><div class="output" id="out-seckey">—</div><button class="copy-btn" data-target="out-seckey">copy</button></div>
    </div>
  </div>
  ...
  <div id="panel-sign" class="panel">
    ...
    <div class="field">
      <label>secret key (base64) または seed (hex 64文字)</label>
      <textarea id="sign-seckey" placeholder="秘密鍵のBase64、またはseedのhex"></textarea>
    </div>
  ...
  <div id="panel-verify" class="panel">
    ...
    <div class="field"><label>signature (base64)</label><textarea id="verify-signature" placeholder="署名のBase64"></textarea></div>
    <div class="field"><label>public key (base64)</label><textarea id="verify-pubkey" placeholder="公開鍵のBase64"></textarea></div>
  ...
  <!-- sk→pk -->
  <div id="panel-sktopk" class="panel">
    <div class="field">
      <label>secret key (base64)</label>
      <textarea id="sktopk-input" placeholder="秘密鍵のBase64"></textarea>
    </div>
    <div class="field"><button class="action" id="btn-sktopk">公開鍵を導出する</button></div>
    <div id="sktopk-timing" class="timing"></div>
    <div class="field">
      <label>public key (base64)</label>
      <div class="output-wrap"><div class="output" id="out-sktopk">—</div><button class="copy-btn" data-target="out-sktopk">copy</button></div>
    </div>
  </div>
...
$('#btn-keygen').addEventListener('click', () => {
    ...
    requestAnimationFrame(() => {
        try {
            const seedInput = ($('#keygen-seed')).value.trim();
            let seed;
            if (seedInput) {
                if (!isHex64(seedInput)) {
                    showToast('seedは64文字のhexで入力してください');
                    return;
                }
                seed = hexToU8(seedInput);
            }
            const t0 = performance.now();
            const keypair = dilithium.keyGen(seed);
            const ms = (performance.now() - t0).toFixed(1);
            setOutput('out-pubkey', b64encode(keypair.pk));
            setOutput('out-seckey', b64encode(keypair.sk));
            $('#keygen-timing').textContent =
                `${ms}ms  |  pk: ${keypair.pk.length}B  sk: ${keypair.sk.length}B`;
        }
        catch (err) {
...
$('#btn-sign').addEventListener('click', () => {
    ...
    const skInput = ($('#sign-seckey')).value.trim();
    if (!msg || !skInput)
        return;
    const btn = $('#btn-sign');
    btn.setAttribute('disabled', '');
    ...
    requestAnimationFrame(() => {
        try {
            ...
            if (isHex64(skInput)) {
                // seedとして扱い、skを導出
                const seed = hexToU8(skInput);
                const keypair = dilithium.keyGen(seed);
                sk = keypair.sk;
                showToast('seedから鍵を生成しました。keygen タブで公開鍵を保存することをお勧めします');
            }
            else {
                sk = b64decode(skInput);
            }
            const msgBytes = new TextEncoder().encode(msg);
            const t0 = performance.now();
            const signature = dilithium.sign(sk, msgBytes);
            const ms = (performance.now() - t0).toFixed(1);
            setOutput('out-signature', b64encode(signature));
            $('#sign-timing').textContent =
                `${ms}ms  |  sig: ${signature.length}B`;
        }
        catch (err) {
            setOutput('out-signature', 'エラー: ' + err.message);
        }
        finally {
            btn.removeAttribute('disabled');
...
$('#btn-verify').addEventListener('click', () => {
    ...
    const sigStr = ($('#verify-signature')).value.trim();
    const pkStr = ($('#verify-pubkey')).value.trim();
    if (!msg || !sigStr || !pkStr)
        return;
    ...
    requestAnimationFrame(() => {
        try {
            const msgBytes = new TextEncoder().encode(msg);
            const signature = b64decode(sigStr);
            const pk = b64decode(pkStr);
            const t0 = performance.now();
            const valid = dilithium.verify(pk, msgBytes, signature);
            const ms = (performance.now() - t0).toFixed(1);
            $('#verify-timing').textContent = `${ms}ms`;
            $('#verify-result').innerHTML =
                `<span class="result-badge ${valid ? 'valid' : 'invalid'}">${valid ? '有効' : '無効'}</span>`;
        }
        catch (err) {
            $('#verify-result').innerHTML =
                `<span class="result-badge invalid">エラー: ${err.message}</span>`;
        }
        finally {
            btn.removeAttribute('disabled');
...
// sk → pk
// ============================================================
$('#btn-sktopk').addEventListener('click', () => {
    const skStr = ($('#sktopk-input')).value.trim();
    if (!skStr)
        return;
    const btn = $('#btn-sktopk');
    btn.setAttribute('disabled', '');
    btn.textContent = '導出中…';
    requestAnimationFrame(() => {
        try {
            const sk = b64decode(skStr);
            const t0 = performance.now();
            const pk = dilithium.pkFromSk(sk);
            const ms = (performance.now() - t0).toFixed(1);
            setOutput('out-sktopk', b64encode(pk));
            $('#sktopk-timing').textContent = `${ms}ms  |  pk: ${pk.length}B`;
        }
        catch (err) {
            setOutput('out-sktopk', 'エラー: ' + err.message);
        }
        finally {
            btn.removeAttribute('disabled');
            btn.textContent = '公開鍵を導出する';
        }
    });
});
export {};
