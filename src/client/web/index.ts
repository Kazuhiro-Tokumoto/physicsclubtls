...
const test2 = async () => {
  ...
  for (const key of rootkey) {
    ...
    const pubrawbigint: [bigint, bigint] = [slice1tobigint, slice2tobigint];
...
// チェーンが正しいかだけ
const verifyCertificate = (cert: Certificate): boolean => {
  const p256 = new p_256();
  const encoder = new TextEncoder();

  const verifyCAChain = (cas: CAEntry[], parentPubkey: string): boolean => {
    for (const ca of cas) {
      ...
      if (ca.end || ca.domain) continue; // ← ここだけ変更
...
const verifyDomain = (cert: Certificate, targetDomain: string): boolean => {
  const p256 = new p_256();
  const encoder = new TextEncoder();
  const now = Date.now();
    ...
    for (const ca of cas) {
      if (ca.domain) {
        for (const domain of ca.domain) {
          if (domain.domainname !== targetDomain) continue;
          const signDate = new Date(domain.signday).getTime();
          if (now - signDate > threeMonths) return false;
          return p256.verify(
...
const test4 = async () => {
  ...
  const rootSign = p256test.sign(
    encoder.encode(caKP.publicKey),
    rootKP.privateKey,
  );

  // CAテスト: チェーンのみ
  const certCA: Certificate = {
    root: [
      {
        keynumber: 0,
        ...
        CA: [
          {
            ...
            caname: "",
            signature:
              "4C068212AD1D563ED4E6C1B9A06AE1351F47E480047BDF8502A51D277F3F94ADB5774F8D3623F1545871ABF34CAC868648CA50194216021F00FA634FEEA23BAC",
            end: true,
            domain: [
              {
  ...
  console.log(
    "ドメイン検証 (example.com):",
    verifyDomain(certDomain, "example.com"),
    "← trueが正解",
  );
  console.log(
    "ドメイン検証 (other.com):",
    verifyDomain(certDomain, "other.com"),
    "← falseが正解",
  );
