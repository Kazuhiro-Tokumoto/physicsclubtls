...
private readonly GAMMA1 = 131072n;        // 2^17
private readonly GAMMA2 = 95232n;         // (Q-1)/88
private readonly OMEGA = 80;
private readonly BETA = 78n;              // TAU * ETA = 39 * 2
private readonly TAU = 39;
private readonly LAMBDA = 128;
...
private readonly Z_POLY_BYTES = 576;  // N * 18 / 8
...
private unpackZ(buf: Uint8Array): bigint[][] {
  ...
  for (let i = 0; i < this.L; i++) {
    const p = new Array<bigint>(this.N);
    for (let j = 0; j < this.N; j += 4) {
      const b = buf.slice(off, off + 9); off += 9;
...
public pkFromSk(sk: Uint8Array): Uint8Array {
...
// タブ
// ============================================================
$('#tab-bar').addEventListener('click', (e: MouseEvent) => {
  const btn = (e.target as Element).closest('.tab') as HTMLElement;
  if (!btn) return;
  const tab = btn.dataset['tab']!;
  $$<HTMLElement>('.tab').forEach(t => t.classList.remove('active'));
  $$<HTMLElement>('.panel').forEach(p => p.classList.remove('active'));
  btn.classList.add('active');
  (document.getElementById(`panel-${tab}`) as HTMLElement).classList.add('active');
});
...
document.addEventListener('click', (e: MouseEvent) => {
  const btn = (e.target as Element).closest('.copy-btn') as HTMLElement;
  if (!btn) return;
...
$('#btn-keygen').addEventListener('click', () => {
  ...
  requestAnimationFrame(() => {
    try {
      const seedInput = ($<HTMLInputElement>('#keygen-seed')).value.trim();
      ...
      ($('#keygen-timing') as HTMLElement).textContent =
...
$('#btn-sign').addEventListener('click', () => {
  const msg = ($<HTMLTextAreaElement>('#sign-message')).value;
  const skInput = ($<HTMLTextAreaElement>('#sign-seckey')).value.trim();
  if (!msg || !skInput) return;
  ...
  requestAnimationFrame(() => {
    try {
      ...
      ($('#sign-timing') as HTMLElement).textContent =
...
$('#btn-verify').addEventListener('click', () => {
  const msg = ($<HTMLTextAreaElement>('#verify-message')).value;
  const sigStr = ($<HTMLTextAreaElement>('#verify-signature')).value.trim();
  const pkStr = ($<HTMLTextAreaElement>('#verify-pubkey')).value.trim();
  if (!msg || !sigStr || !pkStr) return;
  ...
  requestAnimationFrame(() => {
    try {
      ...
      ($('#verify-timing') as HTMLElement).textContent = `${ms}ms`;
      ($('#verify-result') as HTMLElement).innerHTML =
        `<span class="result-badge ${valid ? 'valid' : 'invalid'}">${valid ? '有効' : '無効'}</span>`;
    } catch (err: any) {
      ($('#verify-result') as HTMLElement).innerHTML =
        `<span class="result-badge invalid">エラー: ${(err as Error).message}</span>`;
    } finally {
      btn.removeAttribute('disabled');
...
$('#btn-sktopk').addEventListener('click', () => {
  const skStr = ($<HTMLTextAreaElement>('#sktopk-input')).value.trim();
  if (!skStr) return;
  ...
  requestAnimationFrame(() => {
    try {
      ...
      ($('#sktopk-timing') as HTMLElement).textContent = `${ms}ms  |  pk: ${pk.length}B`;
    } catch (err: any) {
