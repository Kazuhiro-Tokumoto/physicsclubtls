...
export class ecsh {
  ...
  private readonly G_precomp_window: [bigint, bigint, bigint][][] = (() => {
    ...
    for (let i = 0; i < 32; i++) {
      ...
      row[0] = [0n, 1n, 0n];
      row[1] = base;
      for (let j = 2; j < 256; j++) row[j] = this.addPointsJacobian(row[j - 1], base);
      table.push(row);
  ...
  private doubleJacobian(Pt: [bigint, bigint, bigint]): [bigint, bigint, bigint] {
    const [X, Y, Z] = Pt;
    if (Z === 0n) return Pt;
    const p    = this.P;
    const YY   = Y * Y % p;
  ...
  private scalarMultG(k: bigint): [bigint, bigint] {
    ...
    for (let i = 0; i < 256; i += 8) {
      ...
      if (win > 0) R = this.addPointsJacobian(R, this.G_precomp_window[i >> 3][win]);
    }
  ...
  public scalarMult(k: bigint, Pt: [bigint, bigint]): [bigint, bigint] {
    ...
    tbl[0] = [0n, 1n, 0n];
    tbl[1] = baseJ;
    for (let i = 2; i < ws; i++) tbl[i] = this.addPointsJacobian(tbl[i - 1], baseJ);
  ...
  private shamirsMult(
    k1: bigint, _P1: [bigint, bigint],
    k2: bigint, P2:  [bigint, bigint]
  ): [bigint, bigint] {
    // G側: 事前計算テーブル (ダブリング0回)
    let R1: [bigint, bigint, bigint] = [0n, 1n, 0n];
    for (let i = 0; i < 256; i += 8) {
      const win = Number((k1 >> BigInt(i)) & 0xffn);
      if (win > 0) R1 = this.addPointsJacobian(R1, this.G_precomp_window[i >> 3][win]);
    }
    // Q側: w=5 固定ウィンドウ
    const w    = 5;
    const ws   = 1 << w;
  ...
  public sha256(data: Uint8Array): Uint8Array {
    ...
    blocks.set(data); blocks[len] = 0x80;
  ...
  // ─── 署名 ────────────────────────────────────────────────────────────────
  private signtobigint(message: Uint8Array, privateKey: string): { R: [bigint, bigint]; s: bigint } {
  ...
  private _profTmp = 0;
  private _profStart(_n: string) { this._profTmp = performance.now(); }
  private _profEnd(n: string) { this._prof[n] = (this._prof[n] ?? 0) + performance.now() - this._profTmp; }

  public verify(message: Uint8Array, signature: string, publicKey: string): boolean {
  ...
  public privateKeyToPublicKey(hex: string): { compressed: string; uncompressed: string } {
  ...
  public addPoints(P1: [bigint, bigint], Q: [bigint, bigint]): [bigint, bigint] {
  ...
  private modSqrt(n: bigint): bigint {
    if (n === 0n) return 0n;
    if (this.modPow(n, (this.P - 1n) / 2n, this.P) !== 1n) throw new Error("平方根が存在しません");
    ...
    while (true) {
      ...
      M = i; c = b * b % this.P; t = t * b * b % this.P; R = R * b % this.P;
    }
  }

  private bigintToHex(n: bigint): string { return n.toString(16).toUpperCase().padStart(64, "0"); }
  private hexToBigInt(hex: string): bigint { return BigInt("0x" + hex); }
  ...
  private getRandomBigInt(max: bigint): bigint {
    ...
    let r: bigint;
    do { const b = new Uint8Array(bytes); globalThis.crypto.getRandomValues(b); r = this.bytesToBigInt(b); } while (r >= max);
    return r;
  }
}
